...
class _ExhibitionBottomSheetState extends State<ExhibitionBottomSheet>
  ...
  double get headerTopMargin =>
      lerp(min: 20, max: 20 + MediaQuery.of(context).padding.top);

  double get headerFontSize => lerp(min: 14, max: 24);

  double get itemBorderRadius =>
      lerp(min: 8, max: 24); //<-- increase item border radius

  double get iconSize =>
      lerp(min: iconStartSize, max: iconEndSize); //<-- increase icon size

  // calculate top margin based on header margin,
  // and size of all of icons above (from small to big)
  double iconTopMargin(int index) =>
      lerp(
        min: iconStartMarginTop,
        max: iconEndMarginTop + index * (iconsVerticalSpacing + iconEndSize),
      ) +
      headerTopMargin;

  double iconLeftMargin(int index) => lerp(
        min: index * (iconsHorizontalSpacing + iconStartSize),
        max: 0,
      ); //<-- calculate left margin (from big to small)

  double get iconLeftBorderRadius =>
      itemBorderRadius; //<-- Left border radius stays the same

  double get iconRightBorderRadius =>
      lerp(min: 8, max: 0); //<-- Right border radius lerps to 0 instead.
  ...
  // lerp any value based on the controller
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        return Positioned(
          height: lerp(min: minHeight, max: maxHeight),
          left: 0,
          right: 0,
          bottom: 0,
    ...
    return Positioned(
      ...
      child: ClipRRect(
        ...
        child: Image.asset(
          ...
          fit: BoxFit.cover,
          alignment: Alignment(lerp(min: 1, max: 0), 0),
        ),
      ),
    );
  }
